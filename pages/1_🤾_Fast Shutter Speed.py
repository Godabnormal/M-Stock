...
st.write("### 🤾 Fast Shutter Speed in Photography")
...
def generate_result(prediction, testing_image_name):
	...
	class_names = ['Bad', 'Excellence', 'Good']
	...
	if np.argmax(score)==1:
		...
		save_img("E:/Jupyter/temp_dir/"+str(chapter)+"/Excellent/"+str(test_name), img_array)
	elif np.argmax(score)==2:
		st.write("## 🎯 Result is :blue[{}] 👍" .format(class_names[np.argmax(score)]))
		save_img("E:/Jupyter/temp_dir/"+str(chapter)+"/Good/"+str(test_name), img_array)
	else:
		st.write("## 🎯 Result is :red[{}] 👎" .format(class_names[np.argmax(score)]))
		save_img("E:/Jupyter/temp_dir/"+str(chapter)+"/Bad/"+str(test_name), img_array)
